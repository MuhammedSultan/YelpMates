<?php 
 ...
 //   include('inJacksonville.php');
   ...
   include('../apiRequests/iJDetails12.php');
...
// LOCATION
   ...
   $latitude = $responseArray->businesses[12]->coordinates->latitude;
   $longitude = $responseArray->businesses[12]->coordinates->longitude; 
...
//business address
if(isset($responseArray->businesses[12]->location->display_address[0])){
    $addressZero = $responseArray->businesses[12]->location->display_address[0];
}else{
    $addressZero = "This business has does not have an address.";
}

if(isset($responseArray->businesses[12]->location->display_address[1])){
   $addressOne = $responseArray->businesses[12]->location->display_address[1];
}else{
    $addressOne = "";
}

//phone number
$phone = $responseArray->businesses[12]->display_phone;


//review count
if(isset($responseArray->businesses[12]->review_count)){
   $reviewCount = $responseArray->businesses[12]->review_count;
}else{
    $reviewCount = "The reviews are not available for this business.";
}

//overall business rating
if(isset($responseArray->businesses[12]->rating)){
  $businessRating = $responseArray->businesses[12]->rating; 
}else{
